...
function fetchgit(fileName, url, rev, branch, supportHydra) {
  return `    {
    name = "${fileName}";
    path =
      let${supportHydra ? `
...
function fetchLockedDep(supportHydra) {
  return function (pkg) {
    ...
    if (url.startsWith('git+')) {
      ...
      return fetchgit(fileName, urlForGit, rev, branch || 'master', supportHydra)
    }
...
// Object -> String
function generateNix(pkgs, supportHydra) {
  const nameWithVersionAndPackageNix = R.map(fetchLockedDep(supportHydra), pkgs)
