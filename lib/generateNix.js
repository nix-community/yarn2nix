...
function replaceShorthands(url) {
  if (/^[^:@%/\s.-][^:@%/\s]*[/][^:@\s/%]+(?:#.*)?$/.test(url))
    return `git+https://github.com/${url}`
  else if (url.startsWith("github:"))
    return `git+https://github.com/${url.substring(7)}`
  else if (url.startsWith("bitbucket:"))
    return `git+https://bitbucket.com/${url.substring(10)}`
  else if (url.startsWith("gitlab:"))
    return `git+https://gitlab.com/${url.substring(7)}`
  else
    return url
}
...
function fetchLockedDep(builtinFetchGit) {
  return function (pkg) {
    ...
    const [url, sha1OrRev] = replaceShorthands(resolved).split('#')
